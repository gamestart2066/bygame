...
6. **🚫 禁止在代码中散写场景坐标**
   ...
   - ColorBlock 是明确例外：数量、类型与空位来自 `play/config/all_levels_simple_edited.json`，每条 `LevelGrids.json.levels` 通过自身 `layout` 字段直接引用布局；单元使用自然数类型码（`0=空、1=normal、2=unknown、3=boxes`），最大 7×7 且列数必须为奇数，空位位置不限但所有非空节点必须四方向连通，以 Startgridpos 为底部中心按 CFG 间距运行时生成；7×7 中每个空白单元实例化 `rect.prefab`，rect 之间无缝，与 ColorBlock/Boxes 之间保留网格间距
