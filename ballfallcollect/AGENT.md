...
6. **🚫 禁止在代码中散写场景坐标**
   - V 槽、EntranceGate、Startgridpos、BoxCollectPos 等空间基准由用户在 Prefab 中摆放
   - ColorBlock 是明确例外：数量、类型与空位来自 `play/config/all_levels_simple_edited.json`，每条 `LevelGrids.json.levels` 通过自身 `layout` 字段直接引用布局；单元使用自然数类型码（`0=空、1=normal、2=unknown、3=boxes`），最大 7×7 且列数必须为奇数，空位位置不限但所有非空节点必须四方向连通，以 Startgridpos 为底部中心按 CFG 间距运行时生成；固定 7×7 空白拓扑由单一 `RectFillLayer/Graphics` 绘制为无接缝连续面板，与 ColorBlock/Boxes 之间保留网格间距
