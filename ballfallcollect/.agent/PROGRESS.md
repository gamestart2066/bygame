...
- **配置驱动关卡**：`play/config/LevelGrids.json` 是网格、颜色/箱序的唯一事实源；当前 20 关优先使用对称矩形网格并按颜色与箱序复杂度递增
...
- **轨道衔接与节奏**：EntranceGate 与轨道上沿保留可调间隙；真实 Ball 切到上层后两段 Tween 跳入移动槽位；全部 ColorBlock 点击后切换为基础速度 2 倍
