...
- **清轨道道具**：HUD `BtnProp1` 可锁定输入后清空所有占槽球；按颜色跨行预订任意未满 CollectBox 的可编辑槽位，原子性释放轨道并播放错峰弹起/弧线飞入动画，全部到位后恢复操作
- **收纳箱洗牌道具**：HUD `BtnProp2` 对每个至少两行的列，从现存第 2~4 行随机抽取一箱与首箱交换；两箱沿相反侧向弧线换位，期间锁定玩家输入和普通玩法调度
- **ColorBlock 移除道具**：HUD `BtnProp3` 进入选择模式，临时隐藏未解锁 Lid 并揭示 Unknown 真实颜色；选中一个未点击的在场 ColorBlock 后，其 9 球从 Slot 位置实体化并直接飞入同色 CollectBox，其余格恢复选择前状态；Boxes 容器与尚未派发格子不参与选择
